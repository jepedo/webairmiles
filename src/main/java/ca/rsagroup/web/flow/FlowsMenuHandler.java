...
import ca.rsagroup.domain.Identifiable;
import ca.rsagroup.printer.TypeAwarePrinter;
...
public class FlowsMenuHandler implements Serializable {
    ...
    public void removeMenu() {
        String flowId = getFlowModuleId();
        String flowExecutionId = getFlowExecutionId();
    ...
    // -----------------------------------------------
    @Inject
    private TypeAwarePrinter typeAwarePrinter;

    @SuppressWarnings("rawtypes")
    public void updateMenu(Object object) {
        Serializable entityId = null;
        if (object instanceof Identifiable) {
            entityId = ((Identifiable) object).getId();
        }
        updateMenu(entityId, typeAwarePrinter.print(object));
    }

    /**
     * Just update the step of the flow.
     */
    public void updateMenuFromSubFlow() {
        updateMenu(null, null);
    }

    private void updateMenu(Serializable entityId, String label) {
        String flowId = getFlowModuleId();
        String flowExecutionId = getFlowExecutionId();
        ...
        if (label != null) {
            label = flowId + ": " + label;
        } else if (flowMenuEntry.getLabel() == null) {
            label = flowId;
        } else {
            label = flowMenuEntry.getLabel();
        }
    ...
    public String endStateRedirect() {
        String flowId = getFlowModuleId();
     ...
     */
    public boolean isFlowAlreadyOpen(Identifiable<?> entity) {
        String flowId = getFlowModuleId();
     ...
     */
    public String endStateRedirectToAlreadyOpenFlow(Identifiable<?> entity) {
        String flowId = getFlowModuleId();
...
//    }
    ...
    private String getFlowModuleId() {
        RequestContext context = RequestContextHolder.getRequestContext();
        return context != null ? substringBefore(context.getFlowExecutionContext().getDefinition().getId(), "/") : null;
    }

    /**
     * Return the execution id, for example: 'e4'.
     */
    private String getFlowExecutionId() {
        RequestContext context = RequestContextHolder.getRequestContext();
        return context != null ? substringBeforeLast(context.getRequestParameters().get("execution"), "s") : null;
    }
    ...
    private boolean maxExecutionsExceeded() {
        return maxExecutions != null && maxExecutions > 0 && flowMenuEntries.size() > maxExecutions;
    }
